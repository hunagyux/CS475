...
int main( int argc, char *argv[ ] ) {
   ...
   for( int t = 0; t < NUMTRIES; t++ ) {
      ...
      for( int n = 0; n < NUMTRIALS; n++ ) {
         ...
         // If it hits the circle get first intersection:
         if(d >= 0) {
            d = sqrt( d );
            float t1 = (-b + d ) / ( 2.*a ); // time to intersect the circle
            float t2 = (-b - d ) / ( 2.*a ); // time to intersect the circle
            tmin = t1 < t2 ? t1 : t2; // only care about the first intersection
         }

         // If it hits circle and not engulfed then get reflection vector and check if oriented toward plate.
   ...
   //printf("# Threads = %d, # Trials = %d, Prob to hit = %8.2lf, Performace = %8.2lf\n", NUMT, NUMTRIALS, currentProb, maxPerformance);
   printf("%8.2lf, %8.2lf, ", maxPerformance, currentProb);
